...
| RSK-6 | An unbounded `SELECT` hangs the client | Medium | Medium | PR-4.2 streaming and paging — **verified 2026-08-06**, the driver streams. PR-3.5 cancellation was the other half and **does not work**: `OdbcCommand.Cancel()` does not reach the server, measured against both a sorting and a scanning statement. So the mitigation is currently half-built, and a runaway `SELECT` can only be waited out. Resolving PR-3.5 is what closes this |
